...
pub fn parse_feature_with_language(content: &str, language: Option<&str>) -> Result<ast::Feature, ParseError> {
    let pairs = GherkinParser::parse(Rule::feature, content)?;
    let mut feature = ast::Feature::default();
    feature.language = language.map(|s| s.to_string());
        ...
        for pair in feature_pair.into_inner() {
            match pair.as_rule() {
                ...
                Rule::scenario_outline => {
                    ...
                    feature.scenarios.push(ast::Scenario::Outline(scenario_outline));
                }
                Rule::rule => {
...
fn parse_background(pair: pest::iterators::Pair<Rule>, language: Option<&str>) -> Result<ast::Background, ParseError> {
...
fn parse_scenario(pair: pest::iterators::Pair<Rule>, language: Option<&str>) -> Result<ast::Scenario, ParseError> {
    ...
    if let ast::Scenario::Simple(ref mut simple) = scenario {
        for inner_pair in pair.into_inner() {
            match inner_pair.as_rule() {
                Rule::tags => {
                    for tag_pair in inner_pair.into_inner() {
                        if tag_pair.as_rule() == Rule::tag {
                            let tag = tag_pair.as_str().trim_start_matches('@').to_string();
                            simple.tags.push(tag);
                        }
                    }
                }
                Rule::scenario_name => {
...
fn parse_scenario_outline(pair: pest::iterators::Pair<Rule>, language: Option<&str>) -> Result<ast::ScenarioOutline, ParseError> {
...
fn parse_step(pair: pest::iterators::Pair<Rule>, language: Option<&str>) -> Result<ast::Step, ParseError> {
...
fn parse_rule(pair: pest::iterators::Pair<Rule>, language: Option<&str>) -> Result<ast::Rule, ParseError> {
    ...
    for inner_pair in pair.into_inner() {
        match inner_pair.as_rule() {
            ...
            Rule::background => {
                let _background = parse_background(inner_pair, language)?;
            }
            Rule::scenario => {
            ...
            Rule::scenario_outline => {
                ...
                rule.scenarios.push(ast::Scenario::Outline(scenario_outline));
            }
            _ => {}
        }
    }
